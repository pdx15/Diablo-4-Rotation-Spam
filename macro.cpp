...
bool CheckPixelColor(int x, int y, COLORREF targetColor, int tolerance) {
    ...
    int r2 = GetRValue(targetColor), g2 = GetGValue(targetColor), b2 = GetBValue(targetColor);
...
void LoadConfig() {
    ...
    in >> globalHealthCheckEnable >> healthVKey >> healthKeyName >> healthDelayMs >> healthX >> healthY;
    size_t size = 0;
    if (in >> size) {
        spamKeys.clear();
...
void CoreMacroLoop() {
    while (true) {
        if (IsDiabloActive()) {
            ...
            bool isMouseTriggerPressed = (GetAsyncKeyState(currentMouseKey) & 0x8000) != 0;

            if (globalHealthCheckEnable && !isHealthy && isScriptActive && isMouseTriggerPressed) {
                auto elapsedHealth = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastHealthPressed).count();
                if (elapsedHealth >= healthDelayMs) { PressKey(healthVKey); lastHealthPressed = now; }
            }
    ...
    while (true) {
        if (isCapturingCoordinates) {
            if (GetAsyncKeyState(VK_LBUTTON) & 0x8000) {
                POINT p;
                if (GetCursorPos(&p)) {
                    HWND diabloHwnd = FindWindowW(L"Diablo IV Main Window Class", nullptr);
                    if (diabloHwnd) ScreenToClient(diabloHwnd, &p);
                    healthX = p.x; healthY = p.y;
                }
                isCapturingCoordinates = false; SaveConfig(); Beep(800, 150);
            ...
            for (int vk = 1; vk < 256; vk++) {
                ...
                if (GetAsyncKeyState(vk) & 0x8000) {
                    ...
                    if (keyToCaptureType == 0) { toggleHotkey = vk; toggleKeyName = name; }
                    else if (keyToCaptureType == 1) { settingsHotkey = vk; settingsKeyName = name; }
                    else if (keyToCaptureType == 2) { healthVKey = vk; healthKeyName = name; }
                    else if (keyToCaptureType >= 3) {
                        size_t idx = keyToCaptureType - 3;
                        if (idx < spamKeys.size()) { spamKeys[idx].vKey = vk; spamKeys[idx].keyName = name; }
                    }
                    isCapturing = false; keyToCaptureType = -1; SaveConfig(); Beep(600, 100);
                    std::this_thread::sleep_for(std::chrono::milliseconds(400)); break;
                }
            }
        }
        else {
            if (GetAsyncKeyState(toggleHotkey) & 0x8000) {
                isScriptActive = !isScriptActive; Beep(isScriptActive ? 440 : 220, 150);
                std::this_thread::sleep_for(std::chrono::milliseconds(300));
            }
            if (GetAsyncKeyState(settingsHotkey) & 0x8000) {
                showSettingsWindow = !showSettingsWindow; std::this_thread::sleep_for(std::chrono::milliseconds(300));
            }
        }
...
void LoadLanguage() {
    ...
    while (std::getline(in, line)) {
        ...
        if (key == "gameStatus") lang.gameStatus = val;
        else if (key == "scriptStatus") lang.scriptStatus = val;
        else if (key == "healthStatus") lang.healthStatus = val;
        else if (key == "options") lang.options = val;
        else if (key == "healthy") lang.healthy = val;
        else if (key == "lowHp") lang.lowHp = val;
        else if (key == "captureCoordsTitle") lang.captureCoordsTitle = val;
        else if (key == "captureCoordsDesc") lang.captureCoordsDesc = val;
        else if (key == "captureKeyTitle") lang.captureKeyTitle = val;
        else if (key == "captureKeyDesc") lang.captureKeyDesc = val;
        else if (key == "settingsTitle") lang.settingsTitle = val;
        else if (key == "btnToggle") lang.btnToggle = val;
        else if (key == "btnSettings") lang.btnSettings = val;
        else if (key == "combatCondition") lang.combatCondition = val;
        else if (key == "radioLmb") lang.radioLmb = val;
        else if (key == "radioRmb") lang.radioRmb = val;
        else if (key == "chkGlobalHealth") lang.chkGlobalHealth = val;
        else if (key == "lblHealthKey") lang.lblHealthKey = val;
        else if (key == "lblHealTimer") lang.lblHealTimer = val;
        else if (key == "btnPickCoords") lang.btnPickCoords = val;
